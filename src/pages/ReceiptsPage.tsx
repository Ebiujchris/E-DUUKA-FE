...
function groupIntoReceipts(sales: SaleItem[]): ReceiptGroup[] {
  ...
  for (const sale of sorted) {
    ...
    if (existing) {
      ...
      groups.push({
        key: sale.id, customer, staffName: sale.createdByStaffName,
        paymentType: sale.paymentType, date: sale.createdAt,
        items: [sale], total: Number(sale.totalAmount),
      });
    }
  }
  return groups;
    ...
    <PageShell title="Receipts" description="All transaction receipts. Click Print to get a copy.">
      <div className="space-y-5">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <input className="w-64 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm focus:border-brand-400 focus:outline-none"
            ...
            <>
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                {visibleReceipts.map((group) => {
                  ...
                  const dateStr = new Date(group.date).toLocaleString('en-UG', { dateStyle: 'medium', timeStyle: 'short' });
                  return (
                    <div key={group.key} className="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm hover:shadow-md transition-shadow">
